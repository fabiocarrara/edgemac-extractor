...
from extractor import EdgeMACExtractor
...
app = flask.Flask('edgemac-extractor')
CORS(app)
...
def extract_from_image():
    ...
    try:
        ...
        params = dict(augment=augment, invert=invert, sketch=sketch, size=size)
    ...
    features = app.extractor.extract_from_pil(image, **params)
    return flask.jsonify(features.tolist())


@app.route('/extract', methods=['GET'])
def extract_from_url():
    """ Extracts EdgeMAC features given an image URL. """
...
def start_from_terminal(app):
    ...
    app.logger.info('Initializing EdgeMAC Extractor on {}'.format(device))
    app.extractor = EdgeMACExtractor(args.gpu)
